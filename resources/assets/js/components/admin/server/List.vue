            ...
            <div class="columns">
                <div class="column">
                    ...
                    <p class="control has-addons">
                        <input class="input" type="text" placeholder="名称 / 协议 / 地址 / 介绍" v-model="search_query">
            ...
            <tbody>
                <tr v-for="server in servers">
                    ...
                    <th><router-link :to="{
                        ...
                        }">{{ server.region.name }}</router-link></th>
                    <th>{{ server.name }}</th>
                    <th>{{ server.protocol }}</th>
                    <th>{{ server.address }}</th>
                    <th>{{ server.introduction }}</th>
                    <th><router-link :to="{name: 'admin_server_edit', params: {
                        id: server.id
                    }}" class="button is-primary">编辑</router-link>
                        <a :class="{'button is-danger': true, 'is-loading': delete_loading[server.id]}" @click="delete_server(server.id)">删除</a>
                    </th>
                </tr>
            </tbody>
        </table>
    ...
    export default {
        ...
        methods: {
            ...
            delete_server: function (id) {
                ...
                axios.post("/admin/server/delete", {
                    id: id
                }).then(function (response) {
