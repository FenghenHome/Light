<template>
    <div>
        ...
        <form v-else @keydown.enter="save_commodity">
            ...
            <p class="control">
                <input class="input" type="text" placeholder="6" v-model="commodity.price">
            </p>
            ...
            <a :class="{'button': true, 'is-loading': save_loading}" @click="save_commodity">保存</a>
        </form>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            get_detail: function () {
                let self = this
                axios.get('/admin/commodity/detail', {params: {
                    id: this.$route.params.id
                }}).then(function (response) {
                    self.loading = false
                    self.commodity = response.data.commodity
                    self.transfer = self.format_transfer(self.commodity.flow)
                })
            },
            save_commodity: function () {
                let self = this
                this.save_loading = true
                this.commodity.flow = this.transfer.enable * Math.pow(1024, this.transfer.level)
                axios.post("/admin/commodity/update", {
                    id: this.$route.params.id,
